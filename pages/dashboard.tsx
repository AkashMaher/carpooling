...
const DashboardPage: NextPage = () => {
  ...
  const checkUser = async ()=> {
    ...
    const handleClaim = async () => {
        ...
        const claim = await carContract.claim()
        .then((tx: any) => {
    ...
    <main className="p-4 pt-6 lg:px-16 min-h-screen">
      {Loading && <p className='text-center'>Loading...</p>}
       ...
       <div>
        ...
        {
          isActiveRide && <button className="outline-none mr-4 mt-4 w-30 h-full bg-[#585858] py-[1%] px-[9.5%] text-white rounded-lg" onClick={()=> {router.push('./rides/active')}}>Current Ride</button>
        } 
        { 
