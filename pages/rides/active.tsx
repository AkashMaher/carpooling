...
const AccountPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    const results:any = await directionsService.route({
      origin: origin, destination: destination, // eslint-disable-next-line no-undef
    ...
    <main className="p-4 pt-6 lg:px-16 min-h-screen">
       ...
       <div>
        ...
        <ul>
            ...
            <li><span className='text-blue-500'>Cost :</span> {ride?.costPerKM *ride?.distance} INR</li>
            <li><span className='text-blue-500'>Status:</span> {ride?.status==1?"Requested":ride?.status==2?"Cancelled":ride?.status==3?"Accepted":ride?.status==4?"Completed":''}</li>
            <li><span className='text-blue-500'>Time:</span> {ride?.time}</li>
        </ul>
        </div>
        <br></br>
